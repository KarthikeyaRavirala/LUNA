...
export default function ProfilePage() {
    ...
    const currentPhase = useCycleStore(state => state.currentPhase) || 'luteal';
    const currentDay = useCycleStore(state => state.currentDay) || 19;
    const cycleLength = useCycleStore(state => state.cycleLength) || 28;
    ...
    const [googleConn, setGoogleConn] = useState({ calendar: true, fit: false, tasks: false, gmail: true });

    const STATS = [
        { label: 'Days Logged', value: '42', icon: '📝' },
        { label: 'Current Streak', value: user.currentStreak || 0, icon: '🔥' },
        { label: 'Moon Dust', value: user.moonDust || 0, icon: '✨' },
        { label: 'Posts Saved', value: '14', icon: '🔖' }
    ];
    ...
    const PHASES_LIST = [
        { key: 'menstrual', label: 'Menstrual', emoji: '🌑', days: 'Days 1-5', color: '#ef5350', bg: 'rgba(239, 83, 80, 0.08)' },
        { key: 'follicular', label: 'Follicular', emoji: '🌱', days: 'Days 6-13', color: '#66bb6a', bg: 'rgba(102, 187, 106, 0.08)' },
        { key: 'ovulatory', label: 'Ovulatory', emoji: '🌕', days: 'Days 14-16', color: '#ffa726', bg: 'rgba(255, 167, 38, 0.08)' },
        { key: 'luteal', label: 'Luteal', emoji: '🌸', days: `Days 17-${cycleLength}`, color: '#f06292', bg: 'rgba(240, 98, 146, 0.08)' }
    ];
        ...
        <div className="page has-bottom-nav" style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            <div style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(13,13,20,.94)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)', padding: '13px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    {user.role === 'admin' && (
                        <button className="btn btn-ghost" style={{ padding: '6px 16px', fontSize: 12, color: '#ffd54f', border: '1px solid currentColor' }} onClick={() => nav('/admin')}>🛡 Admin</button>
                    )}
                ...
                <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
                            ...
                            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Day {currentDay}</div>
                        </div>
                ...
                {tab === 'cycle' && (
                    <div className="anim-fade-up">
                        <div className="card" style={{ padding: 18, marginBottom: 12 }}>
                            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginBottom: 14 }}>🌙 Your {cycleLength}-Day Cycle</div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>
                                {PHASES_LIST.map(ph => (
                                    <div key={ph.key} style={{ padding: 14, background: ph.key === currentPhase ? ph.bg : 'var(--surface)', border: `1px solid ${ph.key === currentPhase ? ph.color + '55' : 'var(--border)'}`, borderRadius: 'var(--r-md)', boxShadow: ph.key === currentPhase ? `0 0 16px ${ph.color}22` : 'none' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                                            <span style={{ fontSize: 18 }}>{ph.emoji}</span>
                                            {ph.key === currentPhase && <span style={{ fontSize: 9, color: ph.color, fontWeight: 700, background: `${ph.color}22`, padding: '1px 7px', borderRadius: 'var(--r-full)' }}>CURRENT</span>}
                                        </div>
                                        <div style={{ fontSize: 13, fontWeight: 600, color: ph.key === currentPhase ? ph.color : 'var(--text)' }}>{ph.label}</div>
                    ...
                    <div className="anim-fade-up">
                        <div className="card" style={{ padding: 18, marginBottom: 12 }}>
                            ...
                            {[
                                ...
                                { icon: '📏', label: 'Cycle Length', value: `${cycleLength} days` },
                            ].map((item, i) => (
