...
export default function OnboardingFlow() {
    ...
    const updateCycleData = useCycleStore(state => state.updateCycleData);
    ...
    const next = () => {
        ...
        updateUserStats({ lunaName: lunaName || 'LunaSister' });
        updateCycleData(cycleLength, dates[0] || new Date().toISOString());
        completeOnboarding();
        nav('/dashboard');
    };
    ...
    return (
        <div className="page" style={{ fontFamily: 'var(--font-sans)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border)' }}>
                ...
                <span className="serif" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 18, color: 'var(--rose-lite)' }}>
                    <img src="/luna-logo.png" alt="Luna Logo" style={{ width: 20, height: 20, borderRadius: '20%' }} /> LUNA
                </span>
